...
class Portfolio_module extends Backend_Controller
{
	...
	public function create ()
	{
        ...
        $this->form_validation->set_rules('limit_projects_number','Limit Projects','integer|numeric');

		if($this->form_validation->run() == FALSE)
		{ 

			//Get Blog Categories
			$this->data['categories'] = $this->portfolio_categories_m->get_portfolio_categories();
			...
			//{
				...
				$this->data['path_to_portfolio_module_layouts'] = APPPATH . 'views/themes/' . $default_template_name . '/modules_layouts/portfolio_modules';
				$extensions = array('php');
				//
				$this->data['portfolio_module_layouts'] = get_filenames_by_extension($this->data['path_to_portfolio_module_layouts'], $extensions);
			...
			$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
       		...
       		// Load the view
			$this->load->view('modules/admin/module_portfolio_create', $this->data);
		} 
			...
			$data = array(
				...
				'module_type' 				=> 'portfolio_module', //portfolio module content
				...
				'limit_projects_number'		=> $this->input->post('limit_projects_number'),
            ...
            // ********************************************************************************** //
            //
            //							Create portfolio_module Data Array
            //
			// ********************************************************************************** //

            //Create Module Portfolio Content Data Array
            foreach ($this->input->post('category') as $category)
			{
				$projects_from_categories = array(
					'module_id'					=> $next_module_id,
					'category_id'				=> $category,
				); 
			
				//Custom Module Insert
		        $this->db->insert('module_portfolio_content',$projects_from_categories);

			};


			$this->session->set_flashdata('message', '<p>Portfolio Module Created Succesfully!</p>');
	...
	public function update ($module_id = NULL)
	{
		...
		{ 
			...
			$this->data['module'] = $this->module_m->get_portfolio_module($module_id);

			//Get Portfolio Categories
			$this->data['categories'] = $this->portfolio_categories_m->get_portfolio_categories();

	    	//Get pages assigned to current menu, based on menu_id on menu_items table for eah raw
	    	$current_portfolio_module_categories = $this->module_m->current_portfolio_module_categories($module_id);

	    	$this->data['current_portfolio_module_categories'] = array();
	    	foreach($current_portfolio_module_categories as $category)
	    	{
	    		$this->data['current_portfolio_module_categories'][] = $category->category_id;
	    	}
			...
			// Load the view
			$this->load->view('modules/admin/module_portfolio_update', $this->data);
		}
			...
			//Portfolio Module Update
	        $this->module_m->update_module_portfolio_content($module_id);


			$this->session->set_flashdata('message', '<p>Portfolio Module Updated Succesfully!</p>');
